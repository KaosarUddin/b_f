...
#def compute_proE_distance(A, B, alpha):
    ...
    # Compute A^alpha and B^alpha
    A_alpha = fractional_matrix_power(A, alpha)
    B_alpha = fractional_matrix_power(B, alpha)
    
    # Compute A^(2*alpha) and B^(2*alpha)
    A_2alpha = fractional_matrix_power(A, 2 * alpha)
    B_2alpha = fractional_matrix_power(B, 2 * alpha)
    
    # Compute tr(A^(2*alpha) + B^(2*alpha) - 2 * tr(A^alpha * B^(2*alpha) * A^alpha))
    term1 = (A_2alpha + B_2alpha)
    term2 = (2*np.real(np.trace(sqrtm(np.dot(A_alpha,np.dot(B_2alpha,A_alpha))))))
    
    # Compute the final expression and take the square root
    result =np.real( np.sqrt(np.trace(term1 - term2) / (alpha ** 2)))
...
results_path = "/mmfs1/home/mzu0014/project1/identification_rates_alpha_pro(100)_rest(.1).txt"
