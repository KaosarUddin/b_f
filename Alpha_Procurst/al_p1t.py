...
def generate_file_paths(base_path, scan_type, num_subjects=30):
    ...
    for subject_id in subject_ids:
        file_path = os.path.join(base_path, subject_id, f'{subject_id}_rfMRI_REST1_{scan_type}_300')
...
base_path='/mmfs1/home/mzu0014/connectomes_300/'
...
alpha_values = [.001,.01,.1,.2,.3,.4,.5,.6,.8,1] # Different regularization strengths to test

# Compute the ID rates for different values of tau
id_rates = compute_id_rate_for_alphas(alpha_values)
#print(f"Alpha: {alpha_values}, ID Rate: {id_rates}")

output_file = '/mmfs1/home/mzu0014/project1/id_rates_alpha_pro1(300)_rest_tao.txt'  # Change this to your preferred path on the server
with open(output_file, 'w') as f:
    for tau, id_rate in id_rates.items():
        f.write(f"Alpha: {alpha_values}, ID Rate: {id_rate}\n")
