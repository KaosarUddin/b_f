...
def remove_yeo_network(connectivity_matrices, yeo_labels, network_id):
    """
    Remove a specific Yeo network (network_id) from the connectivity matrices.
    """
    indices_to_remove = [i for i, label in enumerate(yeo_labels) if label == network_id]
    indices_to_keep = [i for i in range(len(yeo_labels)) if i not in indices_to_remove]
    residual_matrices = [
        np.delete(np.delete(mat, indices_to_remove, axis=0), indices_to_remove, axis=1)
...
# Ablation study for Yeo's networks
num_networks = 7  # Assuming 7 Yeo's networks
residual_id_rates = []

for network_id in range(1, num_networks + 1):
    residual_lr = remove_yeo_network(connectivity_matrices_lr, yeo_labels, network_id)
    residual_rl = remove_yeo_network(connectivity_matrices_rl, yeo_labels, network_id)
    ...
    print(f"Residual ID Rate (excluding network {network_id}): {residual_id_rate}")
...
# Paths to data
base_path = 'D:/Research AU/Python/connectomes_100/'  # Update path as necessary
lr_paths = generate_file_paths(base_path, 'LR')
rl_paths = generate_file_paths(base_path, 'RL')
...
output_file = 'D:/Research AU/Python/residual_id_rates.txt'
with open(output_file, 'w') as f:
    f.write("Residual ID Rates\n")
    f.write("=================\n")
    ...
    for network_id in range(1, num_networks + 1):
        ...
        result_line = f"Residual ID Rate (excluding network {network_id}): {residual_id_rate}\n"
        print(result_line.strip())  # Print to console
        f.write(result_line)  # Save to file
