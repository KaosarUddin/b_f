...
def randomize_labels(matrix, num_networks=7):
    num_nodes = matrix.shape[0]
    labels = np.arange(num_nodes)
    np.random.shuffle(labels)  # Randomize labels
    randomized_matrix = matrix[labels][:, labels]  # Shuffle rows and columns
...
for _ in range(num_iterations):
    randomized_lr = [randomize_labels(mat) for mat in connectivity_matrices_lr]
    randomized_rl = [randomize_labels(mat) for mat in connectivity_matrices_rl]
    bw_distance_matrix_null_lr_rl = BW_distance_matrix(randomized_lr, randomized_rl, alpha, z)
    bw_distance_matrix_null_rl_lr = BW_distance_matrix(randomized_rl, randomized_lr, alpha, z)

    # Compute ID rates for both directions
    id_rate_null_lr_rl = compute_id_rate(bw_distance_matrix_null_lr_rl)
    id_rate_null_rl_lr = compute_id_rate(bw_distance_matrix_null_rl_lr)

    # Average the ID rates
    id_rate_null = (id_rate_null_lr_rl + id_rate_null_rl_lr) / 2
    null_id_rates.append(id_rate_null)

mean_null_id_rate = np.mean(null_id_rates)
std_null_id_rate = np.std(null_id_rates)

# Save results
results_path = "D:/Research AU/Python/null_model_results.txt"
with open(results_path, 'w') as f:
    f.write(f"Original ID Rate: {current_id_rate}\n")
    f.write(f"Null Model Mean ID Rate: {mean_null_id_rate}\n")
    f.write(f"Null Model Std Dev: {std_null_id_rate}\n")
...
#**Inter-network connections unchanged: Randomizing only intra-network connections**___________
...
def randomize_labels_yeo(matrix, yeo_labels):
    ...
    for label in unique_labels:
        ...
        # Shuffle rows and columns for this label
...
# Example Yeo's labels (for demonstration, you need actual labels)
yeo_labels = np.random.randint(1, 8, size=connectivity_matrices_lr[0].shape[0])
...
for iteration in range(num_iterations):
    # Dynamically shuffle Yeo labels for each iteration
    randomized_yeo_labels = yeo_labels.copy()
    np.random.shuffle(randomized_yeo_labels)

    randomized_lr = [randomize_labels_yeo(mat, randomized_yeo_labels) for mat in connectivity_matrices_lr]
    randomized_rl = [randomize_labels_yeo(mat, randomized_yeo_labels) for mat in connectivity_matrices_rl]
    ...
    print(f"Iteration {iteration + 1}: ID Rate = {id_rate_null}")

mean_null_id_rate = np.mean(null_id_rates)
std_null_id_rate = np.std(null_id_rates)

# Save results
results_path = "D:/Research AU/Python/null_model1_results.txt"
with open(results_path, 'w') as f:
    f.write(f"Original ID Rate: {current_id_rate}\n")
    f.write(f"Null Model Mean ID Rate: {mean_null_id_rate}\n")
    f.write(f"Null Model Std Dev: {std_null_id_rate}\n")
...
####Randomize both intra-network and inter-network connections while preserving the Yeo label structure.________
...
def randomize_labels_yeo(matrix, yeo_labels):
    """
    Randomize both intra-network and inter-network connections while preserving the Yeo label structure.
    """
    randomized_matrix = np.zeros_like(matrix)
    unique_labels = np.unique(yeo_labels)
...
base_path = 'D:/Research AU/Python/connectomes_100/'  # Replace with actual path
lr_paths = generate_file_paths(base_path, 'LR')
rl_paths = generate_file_paths(base_path, 'RL')
...
# Save results
results_path = "/mmfs1/home/mzu0014/null_model_results.txt"  # Replace with actual path
with open(results_path, 'w') as f:
    f.write(f"Original ID Rate: {current_id_rate}\n")
    f.write(f"Null Model Mean ID Rate: {mean_null_id_rate}\n")
    f.write(f"Null Model Std Dev: {std_null_id_rate}\n")