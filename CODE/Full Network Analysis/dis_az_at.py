...
for task in tasks:
    ...
    for alpha, z in alpha_z_values:
        lr_paths = generate_file_paths(base_path, task, 'LR', 30)  # Assuming 30 subjects per task for simplicity
        rl_paths = generate_file_paths(base_path, task, 'RL', 30)
...
with open(output_file_path, 'w') as file:
    for task, alpha_z_rates in id_rate_results.items():
        ...
        for (alpha, z), id_rate in alpha_z_rates.items():
            file.write(f"Alpha: {alpha}, Z: {z}, ID Rate: {current_id_rate}\n")
        file.write("\n")

# Plotting
plt.figure(figsize=(10, 8))
alpha_z_combinations = [f"Alpha={alpha}, Z={z}" for alpha, z in alpha_z_values]
for task in tasks:
    id_rates_for_plot = [id_rate_results[task][(alpha, z)] for alpha, z in alpha_z_values]
    plt.plot(alpha_z_combinations, id_rates_for_plot, marker='o', linestyle='-', label=task)

plt.title('Identification Rates Across Tasks and Alpha-Z Configurations')
plt.xlabel('Alpha-Z Configuration')
plt.ylabel('Identification Rate')
plt.xticks(rotation=45)
plt.legend(loc='upper left')
plt.grid(True)

# Save the plot to the server
plot_file_path = '/mmfs1/home/mzu0014/project1/id_rates_az_at_plot.png'
