...
def generate_file_paths(base_path, task, scan_type, num_subjects=30):
    file_paths = []
...
def distance_matrix(connectivity_matrices_1, connectivity_matrices_2, tau=1e-6):
    ...
    for i, matrix_1 in enumerate(connectivity_matrices_1):
        ...
        for j, matrix_2 in enumerate(connectivity_matrices_2):
            ...
            dist_matrix[i, j] = compute_geodesic_distance(matrix_1, matrix_2)
    return dist_matrix
...
for task in tasks:
    for tau_index, tau in enumerate(tau_values):
        for rank_index, rank in enumerate(ranks):
            if tau in id_rate_results_per_task[task] and len(id_rate_results_per_task[task][tau]) > rank_index:
                rate = id_rate_results_per_task[task][tau][rank_index]
                plt.plot(tau, rate, marker='o', linestyle='-', label=f'{task} Tau {tau} Rank {rank}')
...
plt.tight_layout()
plot_file_path = '/mmfs1/home/mzu0014/project1/id_rates_ai_at_comprehensive.png'
plt.savefig(plot_file_path)
plt.close()

output_file_path = '/mmfs1/home/mzu0014/project1/id_rate_ai_at_results_summary.txt'
